...
export default function Home() {
  ...
  return <main>
    ...
    <section className="detail-section" id="detail"><div className="detail-heading"><p className="eyebrow">02 / Detail materi</p><h2>{selected.title}<br /><em>catatan praktik.</em></h2><p>{selected.description}</p></div><div className="detail-body">{selected.image && <Image className="detail-image" src={selected.image} alt={`Alat ${selected.title}`} width={720} height={420} unoptimized />}{selected.readme && <div className="readme-box"><span className="mini-label">CATATAN & KONEKSI</span><p>{cleanMarkdown(selected.readme)}</p></div>}<div className="sketch-list"><div className="mini-label">FILE SKETCH</div>{selected.sketches.map((sketch) => <button type="button" key={sketch.id} className={selectedSketch.id === sketch.id ? "sketch-button active" : "sketch-button"} onClick={() => selectSketch(sketch)}><span>{sketch.title}</span><small>{sketch.filename}</small><b>↗</b></button>)}</div></div></section>

    <section className="code-section" id="kode"><div className="code-heading"><p className="eyebrow">03 / File sketch</p><h2>Lihat kode<br /><em>dan coba di IDE.</em></h2><p>{selectedSketch.filename} · file ini diambil langsung dari materi praktik. Pilih file lain di atas untuk membandingkan eksperimen.</p></div><div className="code-window"><div className="window-bar"><div className="window-dots"><i /><i /><i /></div><span>{selectedSketch.filename}</span><div className="code-actions"><button type="button" onClick={copyCode}>{copied ? "Tersalin!" : "Salin kode"} <span>□</span></button><a href={selectedSketch.path} download={selectedSketch.filename}>Unduh .ino <span>↓</span></a></div></div><pre><code>{selectedSketch.code}</code></pre></div></section>
